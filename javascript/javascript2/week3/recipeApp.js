...
function createRecipeContent(recipe) {
  ...
  recipeContent.innerHTML = `
    ...
    <p class="recipe-meta">
      <span><i class="fas fa-clock"></i> ${recipe.timeRequired || "N/A"} mins</span> 
      <span><i class="fas fa-list"></i> ${recipe.ingredients.length} ingredients</span> 
      <span><i class="fas fa-map-marker-alt"></i> ${recipe.cuisine || "Unknown"}</span>
    </p>
  `;
...
function addRecipeEventListeners(recipeItem, overlay, recipe) {
  recipeItem.addEventListener("mouseover", () => recipeItem.classList.add("hovered"));
  recipeItem.addEventListener("mouseout", () => recipeItem.classList.remove("hovered"));
...
document
  ...
  .addEventListener("change", function (event) {
    ...
    displayRecipes(filteredRecipes);
    document.getElementById("messageFilter").innerText= `# ${event.target.value} Recipes`
  });
...
document
  ...
  .addEventListener("change", function (event) {
    ...
    document.getElementById("messageSort").innerText= `Sorted by:  ${event.target.value} `
...
document.getElementById("startTimer").addEventListener("click",()=> startCookingTimer(6))
